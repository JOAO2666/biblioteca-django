...
from drones.models import DroneCategory, Pilot, Colecao
...
class PilotTests(APITestCase):
    ...
    # Testa a criação de um piloto sem token
...
class ColecaoTests(APITestCase):
    ...
    def test_only_owner_can_edit_colecao(self):
        colecao = Colecao.objects.create(nome='Coleção', descricao='Descrição', colecionador=self.user)
    ...
    def test_non_owner_cannot_edit_colecao(self):
        other_user = User.objects.create_user(username='otheruser', password='otherpass')
        colecao = Colecao.objects.create(nome='Coleção', descricao='Descrição', colecionador=other_user)
        url = reverse('colecao-detail', args=[colecao.id])
        data = {'nome': 'Coleção Editada'}
        response = self.client.put(url, data, format='json')
